// Exercício 02: árvore de busca completa
...
void inserirElemento(Node **raiz, Node **atual){
 // Utilizamos ponteiro duplo caso a função precise alterar as variáveis.
    ...
    if (*raiz == NULL){ //Caso o node seja o primeiro, será a raiz
        ...
        *atual = *raiz; // Atribui o valor atual como raiz, para começar a busca

        while (1)
        {
            if (valorConvertido < (*atual)->valor){

            if ((*atual)->left == NULL)
            {
                ponto->pai = *atual; // Atribui o pai do novo node como o atual
                (*atual)->left = ponto; // Atribui o novo node a esquerda do atual
                break;
            }
            else{
                printf("\nO elemento atual ja possui um valor a esquerda.\n");
                *atual = (*atual)->left; // Atribui o valor atual como o filho a esquerda do atual
            }    
            }
            else if (valorConvertido > (*atual)->valor){

            if ((*atual)->right == NULL)
            {
                ponto->pai = *atual; // Atribui o pai do novo node como o atual
                (*atual)->right = ponto; // Atribui o novo node a esquerda do atual
                break;
            }
            else{
                printf("\nO elemento atual ja possui um valor a direita.\n");
                *atual = (*atual)->right; // Atribui o valor atual como o filho a direita do atual
            }
            } 

            else
            {
                printf("\nO valor ja existe na arvore, tente novamente.\n");
